-- ---------------------------------------------------------------------------
--  TPC-DS indexes for MariaDB
--
--  Taken verbatim from tpcds-run-tool/mariadb-tpcds-tooling2/ddl/indexes.sql
--  (https://github.com/spetrunia/tpcds-run-tool). Validated against this
--  schema: all 117 statements reference existing tables and columns, and no
--  index name is duplicated.
--
--  117 indexes. Most cover a single foreign-key column -- the same columns the
--  foreign keys in 02_foreign_keys.sql would have indexed. A handful are
--  composite indexes covering common filter predicates, e.g.
--      customer_demographics (cd_marital_status, cd_education_status)
--      store_sales (ss_quantity, ss_list_price, ss_coupon_amt, ss_wholesale_cost)
--
--  NOTE on TPC-DS compliance: Clause 2.5.3 restricts Explicit Auxiliary Data
--  Structures. These indexes materialize base-table columns and would need to
--  be justified against 2.5.3.2 for an audited result. They are fine for
--  optimization work, which is what they are used for here.
--
--  Apply AFTER loading data:  mariadb tpcds < sql/05_indexes.sql
-- ---------------------------------------------------------------------------
