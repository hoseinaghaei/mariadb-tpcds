-- ---------------------------------------------------------------------------
--  Hide the three `inventory` secondary indexes from the optimizer.
--
--  These are the measured cause of the worst remaining regressions. Making
--  them IGNORED is metadata-only: it takes ~0.15s, the index is left fully
...
ALTER TABLE inventory ALTER INDEX idx_inv_item_sk      IGNORED;
ALTER TABLE inventory ALTER INDEX idx_inv_warehouse_sk IGNORED;
...
GROUP BY TABLE_NAME, INDEX_NAME, IGNORED;
